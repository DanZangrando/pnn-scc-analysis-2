...
if os.path.exists(raw_data_path):
    ...
    for g in groups:
        ...
        for s in sections:
            ...
            files = sorted([f for f in os.listdir(s_path) if f.lower().endswith(('.tif', '.czi'))])
            for f in files:
                all_tasks.append((g, s, f, os.path.join(s_path, f)))

if not all_tasks:
    st.warning("⚠️ No se encontraron imágenes en la estructura de `data/processed/mips` o `data/raw`.")
    st.stop()
