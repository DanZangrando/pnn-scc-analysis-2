...
st.markdown('<div class="main-header">🧪 Paso 1: Segmentación de Interneuronas PV+</div>', unsafe_allow_html=True)
...
if os.path.exists(seg_file):
    ...
    if st.button("🧪 Abrir en Napari", type="primary", key="p2_btn_napari"):
        cmd = [sys.executable, "src/napari_viewer.py", "--path", seg_file, "--pixel_size", str(px_size), "--step", "pv"]
        try:
            env = os.environ.copy()
            env["DISPLAY"] = os.environ.get("DISPLAY", ":0")
            subprocess.Popen(cmd, env=env)
